...
<div class="modal-dialog no-modal-header" role="document"><div class="modal-content"><div class="modal-body">
    <button type="button" class="close" data-bs-dismiss="modal" aria-hidden="true"><i class="fa fa-2x">&times;</i></button>
    <?php
} else {
    ?><!doctype html>
    ...
    <div id="wrapper">
                    ...
                    <tbody>
                        ...
                        if ($return_rows) {
                            ...
                            foreach ($return_rows as $row) {
                                ...
                                echo '<tr><td colspan="2" class="no-border">#' . $r . ': &nbsp;&nbsp;' . $row->product_code . '</td><td class="no-border text-center">' . $this->sma->formatQuantity($row->quantity) . $row->base_unit_code . ' x ' . $this->sma->formatMoney($row->unit_price) . '</td><td class="no-border"><span class="float-end">' . $this->sma->formatMoney($row->subtotal) . '</span></td></tr>';
                                echo '<tr><td colspan="4" class="no-border" style="padding-top:0;padding-bottom:0;">' . product_name($row->product_name, ($printer ? $printer->char_per_line : null)) . ($row->variant ? ' (' . $row->variant . ')' : '') . '</td></tr>';
                                if (!empty($row->second_name)) {
                                    echo '<tr><td colspan="4" class="no-border" style="padding-top:0;padding-bottom:0;">' . $row->second_name . '</td></tr>';
                                }
        ...
        <div id="buttons" style="padding-top:10px; text-transform:uppercase;" class="no-print">
            ...
            if ($modal) {
                ...
                ?>
                <span class="float-end col-xs-12">
                    <?php
                    if ($pos->remote_printing == 1) {
                        echo '<button onclick="window.print();" class="btn btn-block btn-primary">' . lang('print') . '</button>';
                    } else {
                        echo '<button onclick="return printReceipt()" class="btn btn-block btn-primary">' . lang('print') . '</button>';
                        echo '<button onclick="return openCashDrawer()" class="btn btn-block btn-secondary">' . lang('open_cash_drawer') . '</button>';
                    } ?>
                </span>
                <span class="float-start col-xs-12"><a class="btn btn-block btn-success" href="#" id="email"><?= lang('email'); ?></a></span>
